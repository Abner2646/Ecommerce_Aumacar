...
export default function Template04({ vehiculo, caracteristicas: caracteristicasProp, colores: coloresProp, plantillaMarca = 'plantilla1' }) {
  ...
  // Imágenes para slider (empezar desde la segunda imagen, tomar las siguientes 8)
  const imagenesGenerales = imagenes.sort((a, b) => (a.orden ?? 0) - (b.orden ?? 0));
  const imagenesPrincipales = imagenesGenerales.slice(1, 9);
    ...
    <div className={`${plantillaRootClass} template-04-minimal bg-black`}>
      ...
      <section className="relative h-auto aspect-video md:h-screen w-full overflow-hidden">
        ...
        <div className="relative h-full flex flex-col items-center justify-center px-8">
          <h1
            className={`nombre-vehiculo text-5xl md:text-8xl lg:text-9xl text-white text-center tracking-tight mb-4 md:mb-6 plantilla-marca-${plantillaMarca}`}
          >
            {modelo}
          </h1>
          ...
          {version && (
            <p className="text-base md:text-2xl text-white/80 font-light tracking-wide mb-4 md:mb-8">
              {version}
            </p>
          )}
      ...
      {imagenesPrincipales.length > 0 && (
        <section 
          className="relative h-auto aspect-video md:h-screen w-full overflow-hidden bg-black"
        ...
        >
          ...
          {imagenesPrincipales.length > 1 && (
            <>
              <button
                onClick={prevSlide}
                className="slider-nav-button absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-14 md:h-14 flex items-center justify-center bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white"
              ...
              <button
                onClick={nextSlide}
                className="slider-nav-button absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-14 md:h-14 flex items-center justify-center bg-white/10 backdrop-blur-md border border-white/20 rounded-full text-white"
